...
Will store the user data and will be accessed by the LLM profile service. 
...
1. User submits a chat message via the frontend that calls the LLM profile service.
2. Profile service wraps the message with system context, agent profile info that will contain the agent working area and property listings listings, and the external data-fetching function call instruction and definitions and forwards it to LLM as a single prompt.
...
4. The LLM reply back to the LLM profile service with the response
5. LLM Profile service store the request and the response in its local DB
